...
// after a position is reached - this point is offseted by a small amount
// includes
#include <Arduino.h>
...
void setup() {
  ...
  // random point setting +/- half a turn
  InitPt(pts,MOTOR_STEPS*MICROSTEPS / 2 * -1, MOTOR_STEPS*MICROSTEPS / 2);
  // random offsets 2 steps
  InitPt(offsets,stepOffset *-1.0,stepOffset+1);
  PrintPtArray(pts,"Points");
  PrintPtArray(offsets,"offsets");
  indexPt = 0;
  ptPen = {0,0};
  // set up steppers
...
void loop() {
  ...
  if (!controller.isRunning())
  {
    ...
    // for next point usage
    pts[indexPt] = AddPoints(pts[indexPt],offsets[indexPt]);
  }
}
